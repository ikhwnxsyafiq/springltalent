...
const API_BASE_URL = "https://springltalent-api.onrender.com";
    ...
    <main className="min-h-screen bg-gray-950 text-white p-8">
      ...
      {resume && (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 mt-8">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              <p className="text-gray-400">Resume File</p>

              <p>{resume.file_name}</p>

              <a
                href={`${API_BASE_URL}/api/resume/download/${params.id}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-block mt-4 bg-cyan-600 hover:bg-cyan-700 text-white px-4 py-2 rounded-lg"
              >
                Download Resume
              </a>
            </div>
            ...
            <div>
              <p className="text-gray-400">Candidate ID</p>
              <p>{resume.candidate_id}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {report && (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 mt-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div>
              <p className="text-gray-400">Recommended Role</p>

              <p>{report.recommendation?.recommended_role}</p>
            </div>

            <div>
              <p className="text-gray-400">Career Fit Score</p>

              <p>{report.recommendation?.career_fit_score}%</p>
            </div>
          </div>
        </div>
      )}
